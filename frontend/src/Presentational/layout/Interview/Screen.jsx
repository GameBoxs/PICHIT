...
const Screen = ({number, info}) => {
    ...
    const allScreen = Array(number).fill(0).map((_, idx) => {
        ...
        return <DefaultScreen key={idx} streamManager={info.subscribers[idx]} name={name} session={info.mySessionId}/>
    })
    ...
    return <FullScreen>
        <DefaultScreen key={info.publisher} streamManager={info.publisher} name={info.myUserName} session={info.mySessionId}/>
